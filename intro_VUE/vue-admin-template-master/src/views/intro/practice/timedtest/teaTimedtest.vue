  ...
  <div class="app-container">
    ...
    <div class="show" style="margin-top: 10px;" >
        <el-form :inline="true" class="demo-form-inline" label-width="800px">
                ...
                <el-form-item label-width="150px" label="已发布的课堂练习：" prop="pass" style="margin-left: 10px; float:left">
                        {{this.total}}
                </el-form-item>
                <el-form-item>
                    <el-button style="margin-right: 20px; float:right" type="default" @click="saveOrUpdate()" >发布新的随堂测验</el-button>
                </el-form-item>
        </el-form>
    </div>
    <div class="show">
        ...
        <el-table
            ...
            <el-table-column label="操作" width="180" align="center">
                <template slot-scope="scope">
                    <el-button-group>
                        <el-button type="success" size="mini" icon="el-icon-search" @click="toShowTimedtest(scope.row.id)"></el-button>
                        <el-button type="primary" size="mini" icon="el-icon-edit" @click="saveOrUpdate(scope.row.id)"></el-button>
                        <el-button type="danger" size="mini" icon="el-icon-delete" @click="deleteTimedtest(scope.row.id)"></el-button>
                    </el-button-group>
                </template>
            </el-table-column>
        </el-table>
    ...
    <div>
        ...
        :before-close="handleClose">
            <el-form ref="postForm" :model="timedtestInfo" class="form-container">
                ...
                <el-form-item style="margin-bottom: 5px;" label-width="100px" label="选择章节:" v-if="!updateInfo">
                    <!-- 一级分类 章 -->
                    <el-select
                    v-model="timedtestInfo.chapterId"
                    placeholder="选择章"
                    ...
                    <!-- 二级分类 节 -->
                    <el-select 
                        v-model="timedtestInfo.sectionId" 
                        placeholder="选择节">
                        <el-option
                ...
                <el-form-item style="margin-bottom: 5px;" label-width="100px" label="标题:">
                    <el-input :rows="1" v-model="timedtestInfo.title" type="textarea" class="article-textarea" autosize placeholder="请输入内容"/>
                </el-form-item>

                <el-form-item style="margin-bottom: 5px;" label-width="100px" label="题目量:">
                    <el-input :rows="1" v-model="timedtestInfo.count" type="textarea" class="article-textarea" autosize placeholder="请输入数字"/>
                </el-form-item>

                <el-form-item style="margin-left: 4px;" label-width="100px" label="做题时间:">
                    <!-- <el-radio v-model="radio" label="5">五分钟</el-radio>
                    <el-radio v-model="radio" label="10">十分钟</el-radio> -->
                  <el-date-picker
                    v-model="timedtestInfo.gmtDeadline"
                    type="datetime"
                    placeholder="选择截止时间"
...
import timedtest from '@/api/intro/practice/timedtest'
...
export default {
  ...
  methods: {
    ...
    getList(page=1){
        ...
        timedtest.findAllByTeacher(this.page,this.limit)
        .then(response =>{
            this.list = response.data.timedtestList
    ...
    //跳转到某一随堂测试下
    toShowTimedtest(id){
      console.log("查看跳转的随堂测验"+id);
      this.$router.push({ path:'/practice/showTimedtest/'+id})
    },
    ...
    saveOrUpdate(id){
      ...
      if(id != null){ //进行数据回显
          ...
          timedtest.getTimedtestById(id).then( response=>{
              this.timedtestInfo = response.data.timedtestInfo
              console.log(this.timedtestInfo);
          })
      }
    },
    ...
    //   console.log(nowTime);
    //   this.timedtestInfo.gmtDeadline = nowTime;

      if(this.timedtestInfo.id!=null){
        timedtest.updateTimedtest(this.timedtestInfo).then( () =>{
          this.$message({
              type: 'success',
              message: '更新成功!'
          });
        ...
        timedtest.addTimedtest(this.timedtestInfo).then( () =>{
          this.$message({
              type: 'success',
              message: '添加成功!'
          });
    ...
    deleteTimedtest(id){
      ...
      .then(_ => {
          //确认删除
        timedtest.deleteTimedtestById(id).then( ()=>{
          this.$message({
              type: 'success',
              message: '删除成功!'
          });
