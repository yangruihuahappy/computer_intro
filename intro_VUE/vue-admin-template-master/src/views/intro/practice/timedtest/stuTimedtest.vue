  ...
  <div class="app-container">
    ...
    <div class="search">
      <el-form :inline="true" class="demo-form-inline" label-width="120px">
        ...
        <!-- 标题 -->
        <el-form-item>
          <el-input v-model="timedQuery.title" placeholder="练习标题"/> 
        </el-form-item>
        ...
        <div class="div" v-for="(son,index) in topicList" :key="index">
          ...
          <div v-if="son.type === 0" class="option">
            <li v-for="(op,index1) in cutOffOptions(son.options)" :key="index1" >
              <span>{{op}}</span>
              <input type="radio" :name="son.content"  :value="op" @change="get_radio_value(index)" v-model="checkedValue[index]" >
            </li>
          ...
          <div v-if="son.type === 2" class="shortWritten">
            <input type="text" :name="son.content" @change="get_input_value(index)" v-model="inputValue[index]">
          </div>
...
export default {
  data(){
    return {
      ...
      student: {}, //登陆的学生
      stuTimedtestInfo: {}, //待显示的课后作业内容
      topicList: { //打开的课后作业的题目
        type: 0, //题目默认类型
        content: "", //题目默认内容
        options: "", //题目的选项集
        answer: "", //题目的答案
  ...
  methods: {
    ...
    getList(page=1){
      ...
      console.log(this.timedQuery);
      this.page = page
      this.listLoading = true
      timedtest.findAllByStudent(this.page,this.limit,this.student.id,this.timedQuery)
      .then(response =>{
        this.list = response.data.stuTimedtestPage
    ...
    LevelOneChanged(value){
      console.log(this.timedQuery.chapterId);
    ...
    //点击去完成按钮后 传入id为stu_timedtestId
    toComplete(id){
      this.reviewButton = true, //弹筐里的确定键是否可点
      this.dialogFormVisible = true
      timedtest.findStuTimedtestById(id).then(response =>{
        this.stuTimedtestInfo = response.data.stuTimedtestInfo
    ...
    btnfun(){
      ...
      this.stuTimedtestInfo.stuAnswers = answerStr;
      console.log(this.stuTimedtestInfo);
      timedtest.updateStuTimedtest(this.stuTimedtestInfo).then( ()=>{
         this.$message({
              type: 'success',
              message: '提交成功!'
          });
          this.resetStuTimedtest();
      })
    },

    //重置提交信息
    resetStuTimedtest(){
      this.stuTimedtestInfo = ""
      this.topicList = {
        type: 0, //题目默认类型
        content: "", //题目默认内容
        options: "", //题目的选项集
