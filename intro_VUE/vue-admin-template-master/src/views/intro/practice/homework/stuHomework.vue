  ...
  <div class="app-container">
    ...
    <div class="search">
      <el-form :inline="true" class="demo-form-inline" label-width="120px">
        ...
        <!-- 标题 -->
        <el-form-item>
          <el-input v-model="homeworkQuery.title" placeholder="练习标题"/> 
        </el-form-item>
...
export default {
  data(){
    return {
      ...
      homeworkQuery:{}, //封装查询对象 
      chapterOneList:{}, //一级章节目录
      sectionTwoList:{}, //二级小节目录
      student: {}, //登陆的学生
      stuHomeworkInfo: {}, //待显示的课后作业内容
  ...
  methods: {
    ...
    getList(page=1){
      ...
      console.log(this.homeworkQuery);
      this.page = page
      this.listLoading = true
      homework.findAllByStudent(this.page,this.limit,this.student.id,this.homeworkQuery)
      .then(response =>{
        this.list = response.data.stuHomeworkPage
    ...
    LevelOneChanged(value){
      console.log(this.homeworkQuery.chapterId);
    ...
    //点击去完成按钮后 传入id为stu_homeworkId
    toComplete(id){
      this.reviewButton = true, //弹筐里的确定键是否可点
      this.dialogFormVisible = true
      homework.findStuHomeworkById(id).then(response =>{
        this.stuHomeworkInfo = response.data.stuHomeworkInfo
    ...
    btnfun(){
      ...
      this.stuHomeworkInfo.stuAnswers = answerStr;
      console.log(this.stuHomeworkInfo);
      homework.updateStuHomework(this.stuHomeworkInfo).then( ()=>{
         this.$message({
              type: 'success',
              message: '提交成功!'
          });
          this.resetStuHomework();
      })
    },
